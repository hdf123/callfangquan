...
function getTextBoundingRect(opt) {
  return getBoundingRect(opt.text, opt.font, opt.textAlign, opt.textVerticalAlign, opt.textPadding, opt.textLineHeight, opt.rich, opt.truncate);
}
/**
 * @deprecated
...
function getTextRect(text, font, textAlign, textVerticalAlign, textPadding, rich, truncate, textLineHeight) {
  return getBoundingRect(text, font, textAlign, textVerticalAlign, textPadding, textLineHeight, rich, truncate);
}
...
var LogScale = Scale.extend({
  ...
  getTicks: function () {
    ...
    return map(intervalScaleProto$1.getTicks.call(this), function (val) {
      var powVal = round$2(mathPow$1(this.base, val)); // Fix #4158
  ...
  niceTicks: function (approxTickNum) {
    ...
    var niceExtent = [round$2(mathCeil$1(extent[0] / interval) * interval), round$2(mathFloor$1(extent[1] / interval) * interval)];
    this._interval = interval;
    this._niceExtent = niceExtent;
  },
...
if (!env$1.canvasSupported) {
  ...
  var parsePercent$2 = function (value, maxValue) {
    if (typeof value === 'string') {
      if (value.lastIndexOf('%') >= 0) {
        return parseFloat(value) / 100 * maxValue;
      }
    ...
    if (fill != null) {
      ...
      if (fill instanceof Gradient) {
        ...
        if (fill.type === 'linear') {
          ...
          width /= scale$$1[0] * Z;
          height /= scale$$1[1] * Z;
          var dimension = mathMax$4(width, height);
          shift = 2 * 0 / dimension;
  ...
  ZImage.prototype.brushVML = function (vmlRoot) {
    ...
    if (hasRotation) {
      ...
      var maxX = mathMax$4(p0[0], p1[0], p2[0], p3[0]);
      var maxY = mathMax$4(p0[1], p1[1], p2[1], p3[1]);
      var transformFilter = [];
  ...
  var drawRectText = function (vmlRoot, rect, textRect, fromTextEl) {
    ...
    if (!fromTextEl) {
      ...
      if (textPosition instanceof Array) {
        x = rect.x + parsePercent$2(textPosition[0], rect.width);
        y = rect.y + parsePercent$2(textPosition[1], rect.height);
        align = align || 'left';
      } else {
  ...
  for (var i$1 = 0; i$1 < list.length; i$1++) {
    var proto$2 = list[i$1].prototype;
    proto$2.drawRectText = drawRectText;
    proto$2.removeRectText = removeRectText;
    proto$2.appendRectText = appendRectText;
  }
